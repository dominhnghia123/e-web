...
export default function ViewDetailProduct({params}: {params: {slug: string}}) {
  ...
  useEffect(() => {
    const getProduct = async () => {
      ...
      setSpecifications({
        ...
        memory: data.product.specifications.memory,
        pin: data.product.specifications.pin,
  ...
  const handleUpdateProduct = async () => {
    ...
    try {
      const {data} = await axios.post(
        `${process.env.BASE_HOST}/product/update-product`,
        {
          _id: params.slug[1],
          name: dataInput.name,
          slug: isChangedInputSlug
            ? slugify(dataInput.slug)
  ...
  return (
    <div className={styles.container}>
      ...
      <div className={styles.content_container}>
        ...
        <div className={styles.button_container_detail_page}>
          <Button
